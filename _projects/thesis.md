---
title: "Thesis: Gradient Coherence, Diffs & Pruning"
technologies: [Python, PyTorch, matplotlib]
demo_url: "https://scholarsarchive.byu.edu/cgi/viewcontent.cgi?article=11354&context=etd"
excerpt: "An investigation into the behavior of gradients durning training of a simple CNN image classifier and how it relates to generalization."
published: true
sort_order: 30
---
...
1. Empirically validate coherence as a signal of generalizable learning (vs. memorization).
2. Devise a **parameter‑level** quantity (diffs) that can *rank weights* for pruning.
3. Turn the signal into **practical sparsity**: smaller models with competitive accuracy.
...
$$
\alpha = \frac{\lVert \mathbb{E}_{z}[g_z] \rVert_2}{\mathbb{E}_{z}[\lVert g_z \rVert_2]}
$$

High $\alpha$ indicates many samples are pointing parameters in a similar direction.
...
$$
\mathrm{diff}_p = \frac{1}{|D|}\sum_{z\in D}\mathrm{sign}\big(\nabla_\theta \ell(z)\big)_p
$$

Intuition: if most examples *agree on the direction* a weight should move, that weight is likely contributing to generalizable structure. Large‑magnitude diffs → valuable parameters.

### 2.3 Pruning with diffs

1. Train for a short burn‑in; pick an epoch of high coherence.
2. Freeze weights; compute **diffs** over the training set.
3. **Prune** smallest‑magnitude diffs (globally or per‑layer); optionally fine‑tune.
...
1. **Data‑aware signals beat architecture‑only heuristics.** Diffs leverage the *dataset*—useful in pruning and beyond.
2. **Simple, inspectable metrics scale.** A mean‑sign is cheap, stable, and easy to reason about.
3. **Measure, then modularize.** Find the right signal (coherence), make it local (diffs), then productize (pruning pipelines).
